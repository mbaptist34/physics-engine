...
vec4::vec4(const float values[4]){
...
mat4 getTranslationMatrix(const vec3& v) {
  ...
  return mat4( vec4(1, 0, 0, 0), vec4(0, 1, 0, 0), vec4(0, 0, 1, 0), vec4(v[0], v[1], v[2], 1) );
  
}

mat4 getRotationMatrix(const vec4& v) {
...
mat4 getScaleMatrix(const vec3& v) {
  ...
  return mat4( vec4(v[0], 0, 0, 0), vec4(0, v[1], 0, 0), vec4(0, 0, v[2], 0), vec4(0, 0, 0, 1));
